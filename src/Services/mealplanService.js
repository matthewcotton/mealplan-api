...
exports.add = async function (req, res, next) {
  ...
  const docs = await Recipe.aggregate([{ $match: { user: user._id } }]).sample(
    Number(duration)
  );
...
exports.deleteOne = async (req, res, next) => {
  ...
  try {
    await Mealplan.findOneAndDelete({ _id: ObjectId(req.params.id) });
    res.status(200).send("Meal plan Deleted");
  } catch {
    res.status(500).send("Unable to delete meal plan");
  }
};
